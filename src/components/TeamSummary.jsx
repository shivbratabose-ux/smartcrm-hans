...
import { buildTeamSummary, CALL_TARGET, fmtDays, fmtHours } from "../utils/teamSummary";
                  ...
                  <button type="button" onClick={() => onPickUser(r.user.id)} title={`Open ${r.user.name}'s calendar`}
                    ...
                    <span style={{ textAlign: "left" }}>
                      ...
                      <span style={{ display: "block", fontSize: 10, color: r.done === 0 ? "#B45309" : "var(--text3)", fontWeight: r.done === 0 ? 700 : 400 }}>
                        ...
                        {r.leaveDays - r.adminHours / CALL_TARGET.workHours > 0.001 && <span style={{ color: "#B45309", fontWeight: 600 }}> · {fmtDays(Math.round((r.leaveDays - r.adminHours / CALL_TARGET.workHours) * 10) / 10)}d leave</span>}
                        {r.adminHours > 0 && <span style={{ color: "#6D28D9", fontWeight: 600 }}> · {fmtHours(r.adminHours)} admin</span>}
                    ...
                    <td key={c.key} title={cellTitle(cell)}
                      ...
                      {r.cellLeave[c.key] > 0 && (
                        <span style={{ display: "block", fontSize: 9.5, fontWeight: 700, color: "#B45309" }}>
                          {(() => {
                            const admin = r.cellAdminHours[c.key];
                            const leave = Math.round((r.cellLeave[c.key] - admin / CALL_TARGET.workHours) * 100) / 100;
                            const leaveLabel = leave <= 0 ? "" : c.from === c.to ? (leave >= 1 ? "On leave" : "½ day leave") : `${fmtDays(leave)}d leave`;
                            return [leaveLabel, admin > 0 ? `Admin ${fmtHours(admin)}` : ""].filter(Boolean).join(" · ");
                          })()}
                        </span>
                      )}
                      {r.cellPendingLeave[c.key] > 0 && (
                        <span style={{ display: "block", fontSize: 9.5, fontWeight: 600, color: "var(--text3)", fontStyle: "italic" }} title="Leave / admin work requested, awaiting approval — target still applies">
