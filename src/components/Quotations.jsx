      ...
      {modal&&(
        <Modal title={modal.mode==="add"?"New Quotation":(modal.lockedFinal?"View Quotation (Final – Locked)":"Edit Quotation")} onClose={()=>{setModal(null);setFormErrors({});setForm(BLANK_QUOTE);}} lg footer={<><button className="btn btn-sec" onClick={()=>{setModal(null);setFormErrors({});setForm(BLANK_QUOTE);}}>{modal.lockedFinal?"Close":"Cancel"}</button>{!modal.lockedFinal&&<button className="btn btn-primary" onClick={save}><Check size={14}/>Save Quote</button>}{modal.lockedFinal&&<button className="btn btn-sec btn-sm" onClick={()=>{duplicate(form);setModal(null);}}><Copy size={13}/>Duplicate / Revise</button>}</>}>
          ...
          {formTab==="details"&&(<div>
            ...
            {/* ─────────────────────────────────────────────────────────
                Inline snapshot sections — surface every billing/legal/
                commercial field directly in the Details flow so the
                rep can review and edit everything in one pass without
                hunting through tabs. Each section is a native <details>
                so it can be collapsed independently. The id="vf-X"
                anchors are targeted by the Verify-tab checklist's
          ...
          {formTab==="verify"&&(() => {
            const vs = getVerifyStatus(form);
            // All snapshot fields, grouped — surfaces both required + optional
            // so reviewer can scan everything at a glance before Send.
            const ALL_VERIFY = [
              ["LEGAL & BILLING", [
            ...
            const jumpTo = (key) => {
              setFormTab("details");
              // Defer one frame so the Details tab DOM is in place before we scroll.
              setTimeout(() => {
                ...
                <div style={{background:vs.complete?"#ECFDF5":"#FFFBEB",border:`1px solid ${vs.complete?"#86EFAC":"#FCD34D"}`,padding:"10px 14px",borderRadius:8,marginBottom:14,fontSize:12.5}}>
                  ...
                  <div style={{fontSize:11,color:"var(--text3)"}}>
                    {vs.complete
                      ...
                      : "Click any row below to jump to that field in Details. Required fields must be filled before status can leave Draft."}
                  </div>
                </div>

                {ALL_VERIFY.map(([groupLabel, rows]) => (
                  <div key={groupLabel} style={{marginBottom:14}}>
                    <div style={{fontSize:11,fontWeight:700,color:"var(--text3)",letterSpacing:"0.5px",marginBottom:6}}>{groupLabel}</div>
                    <div style={{border:"1px solid var(--border)",borderRadius:6,overflow:"hidden"}}>
                      {rows.map(([key,label,required],i) => {
                        ...
                        return (
                          <div key={key} onClick={() => jumpTo(key)} style={{display:"grid",gridTemplateColumns:"24px 1.2fr 2fr",gap:10,alignItems:"center",padding:"8px 12px",borderTop:i===0?"none":"1px solid var(--border)",cursor:"pointer",background:isMissing?"#FFFBEB":"transparent",fontSize:12.5}}>
