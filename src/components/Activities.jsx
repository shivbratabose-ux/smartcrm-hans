...
function Activities({activities,setActivities,accounts,contacts,opps,currentUser,files,onAddFile,orgUsers}) {
  ...
  return (
    <div>
      ...
      {modal&&(
        <Modal title={modal.mode==="add"?`New Activity`:`Edit Activity`} onClose={()=>setModal(null)} lg
          footer={<><button className="btn btn-sec" onClick={()=>setModal(null)}>Cancel</button><button className="btn btn-primary" onClick={save}><Check size={14}/>{modal.mode==="add"?"Save Activity":"Update Activity"}</button></>}>
