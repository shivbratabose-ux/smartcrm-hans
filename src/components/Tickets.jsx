...
function Tickets({tickets,setTickets,accounts,orgUsers,currentUser,canDelete}) {
  ...
  return (
    <div>
      ...
      {modal&&(
        <Modal title={modal.mode==="add"?"New Ticket":"Edit Ticket"} onClose={()=>{setModal(null);setFormErrors({});}} lg footer={<><button className="btn btn-sec" onClick={()=>{setModal(null);setFormErrors({});}}>Cancel</button><button className="btn btn-primary" onClick={save}><Check size={14}/>Save Ticket</button></>}>
